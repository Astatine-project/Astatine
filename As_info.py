...
community_rights = 'Mester Satellite, © 2023 - 2024 All rights reserved'
...
class HelpENG(discord.ui.View):
    ...
    async def select_callback(self, select, interaction):
        ...
        await interaction.response.edit_message(embed=help_emb, view=HelpENG())

class HelpRU(discord.ui.View):
    @discord.ui.select(
    ...
    async def select_callback(self, select, interaction):
        ...
        await interaction.response.edit_message(embed=help_emb, view=HelpRU())
        ...
        if getlang is not None:
            for language in Language.select().where(Language.guild_id == ctx.guild.id):
                if language.lang == "ru":
                    ...
                    await ctx.respond(embed=help_emb, view=HelpRU())
                else:
                    help_emb = discord.Embed(title='''
                        \nChoose a category below.
                        \nRequired arguments - ```[]```
                        \nOptional arguments - ```{}```''', colour=0x39d0d6)
                    await ctx.respond(embed=help_emb, view=HelpENG())
        else:
            help_emb = discord.Embed(title='''
                \nChoose a category below.
                \nRequired arguments - ```[]```
